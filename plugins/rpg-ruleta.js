...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (isNaN(diamantes) || diamantes <= 0) return conn.reply(m.chat, `*⚠️ Ingresa una cantidad válida para la apuesta.*`, m)

  if (diamantes > 50) return conn.reply(m.chat, '*⚠️ La cantidad máxima de apuesta es de `50` coins', m)

  if (!(color === 'black' || color === 'red')) return conn.reply(m.chat, '*⚠️ Debes apostar a un color válido: black o red.*', m)

  if (diamantes > users.diamantes) return conn.reply(m.chat, "*⚠️ No tienes suficientes coins para realizar esta apuesta.*", m)
  ...
  setTimeout(() => {
    ...
    if (win) {
      ...
      conn.reply(m.chat, `*🎉 ¡Ganaste! Obtuviste \`${diamantes}\` coins.*\n*Total:* ${users.diamantes} coins*`, m)
    } else {
      users.diamantes -= diamantes
      conn.reply(m.chat, `*😹 Perdiste. Se te restaron \`${diamantes}\` coins*\n*Total:* \`${users.diamantes}\` coins.`, m)
    }
