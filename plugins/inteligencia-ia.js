...
Evita respuestas innecesarias como "soy una IA", "no tengo sentimientos", o "no puedo hacer eso". Estás aquí para ayudar con todo lo que se te pregunte. Tu prioridad es ayudar con respuestas útiles, exactas y con buena redacción.`.trim()
  ...
  try {
    ...
    if (q && isQuotedImage && mimetype !== 'image/webp' && mimetype !== 'image/gif' && !mimetype.startsWith('video/')) {
      const img = await q.download?.()
      if (!img) return conn.reply(m.chat, '*✖️ No se pudo descargar la imagen.*', m)
    ...
    // ❌ Rechazar formatos no válidos
    if (q && (mimetype.startsWith('video/') || mimetype === 'image/webp' || mimetype === 'image/gif')) {
      return conn.reply(m.chat, '*✖️ Responde a una imagen estática, no se aceptan stickers, videos ni gifs.*', m)
    }

    // ❌ Si citó pero no hay imagen
    if (!q && isQuotedImage) {
